...
class IncentivadorUFQuery(ContagemQuery):
    def query(self, **kwargs):
        query = Query().raw_query(func.count(Interessado.CgcCpf.distinct()).label("quantidade"), Interessado.Uf.label("local"))
        query = (query
...
class IncentivadorRegiaoQuery(ContagemQuery):
    def query(self, **kwargs):
        query = Query().raw_query(func.count(Interessado.CgcCpf.distinct()).label("quantidade"), UF.Regiao.label("local"))
        query = (query
...
class ProponenteRegiaoQuery(ContagemQuery):
    def query(self, **kwargs):
        query = Query().raw_query(func.count(Interessado.CgcCpf.distinct()).label("quantidade"),
...
class ProjetoUFQuery(ContagemQuery):
    def query(self, **kwargs):
        query = Query().raw_query(func.count(Projeto.PRONAC.distinct()).label("quantidade"),
